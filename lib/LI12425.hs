...
data Terreno
  ...
  | -- | Água para efeito decorativo, mas onde não se pode construir, nem os inimigos se podem mover.
    Agua
  | -- | Lava para efeito decorativo, mas onde não se pode construir, nem os inimigos se podem mover.
    Lava
    -- | Funcionam como a Agua e a Lava, mas também regeneram a vida de inimigos próximos.
  | LagrimasCristalinas
  deriving (Eq, Show)
...
data Torre = Torre
  { -- | Posição da torre no mapa.
    ...
    -- | Probabilidade da torre de errar o ataque.
    probabilidadaErrar :: Probabilidade,
    -- | Probabilidade da torre de dar um ataque crítico (dobro do dano).
    probabilidadaCritico :: Probabilidade,
    -- | Nível da torre.
    nivelTorre :: Int,
    -- | Número da torre.
    numeroTorre :: Int
...
data TipoProjetil = Fogo | Gelo | Resina | Fraqueza
  deriving (Eq, Show)
...
data Inimigo = Inimigo
  { -- | Posição do inimigo no mapa.
    ...
    -- | Imunidade a projéteis.
    imunidadeInimigo :: [TipoProjetil],
    -- | Vida máxima (inicial) do inimigo.
    vidaMaxima :: Float,
    -- | Número do inimigo.
    numeroInimigo :: Int
  }
...
data Jogo = Jogo
  { -- | Base de operações do jogador.
    ...
    posicaoTeclado :: Posicao,
    -- | Quantidade de inimigos derrotados no nível.
    inimigosDerrotados :: Int,
    -- | Estado da chuva no jogo.
    estadoChuva :: (Bool,Tempo,[Posicao]),
    -- | Tempo do dia do jogo.
    tempoDia :: Tempo
  }
  deriving (Show)
